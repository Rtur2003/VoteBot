...
class ControlPanelView(ttk.Frame):
    ...
    def _build(self):
        ...
        title = ttk.Label(header, text="VOTRYX Control Panel", style="Title.TLabel")
        title.grid(row=0, column=0, sticky="w")
        subtitle = ttk.Label(
            header,
            text="Automated voting controller",
        ...
        stats_wrapper = ttk.LabelFrame(self, text="Status", style="Panel.TFrame", padding=10)
        ...
        self.app._make_stat_card(stats_frame, 0, 0, "Votes", "0", "count")
        self.app._make_stat_card(stats_frame, 0, 1, "Errors", "0", "errors")
        self.app._make_stat_card(stats_frame, 0, 2, "State", "Idle", "status")
        self.app._make_stat_card(stats_frame, 0, 3, "Runtime", "00:00:00", "runtime")

        settings_frame = ttk.LabelFrame(self, text="Settings", style="Panel.TFrame", padding=12)
        ...
        general_block = ttk.LabelFrame(
            settings_frame, text="General", style="Panel.TFrame", padding=10
        )
        ...
        self.app.pause_entry = add_field(
            0, 0, "Vote Interval (sec)", self.app.pause_between_votes, "Delay between votes"
        )
        self.app.batch_entry = add_field(
            0, 2, "Batch Size", self.app.batch_size, "Votes per batch"
        )
        self.app.timeout_entry = add_field(
            1, 0, "Timeout (sec)", self.app.timeout_seconds, "Vote button wait limit"
        )
        self.app.max_errors_entry = add_field(
            1, 2, "Max Errors", self.app.max_errors, "Backoff after consecutive errors"
        )
        self.app.backoff_entry = add_field(
            2, 0, "Backoff (sec)", self.app.backoff_seconds, "Initial backoff delay"
        )
        self.app.backoff_cap_entry = add_field(
            2, 2, "Backoff Cap", self.app.backoff_cap_seconds, "Max backoff delay"
        )
        self.app.parallel_entry = add_field(
            3, 0, "Parallel Windows", self.app.parallel_workers, "Browser count"
        )
        ...
        ttk.Label(
            toggles,
            text="Headless mode hides Chrome windows. Selenium Manager auto-resolves drivers.",
            style="Helper.TLabel",
            wraplength=420,
        ).grid(row=1, column=0, columnspan=2, sticky="w", pady=(2, 0))

        advanced_block = ttk.LabelFrame(
            settings_frame, text="Advanced", style="Panel.TFrame", padding=10
        )
        ...
        ttk.Label(
            advanced_block,
            text="Uses custom list or defaults when enabled.",
            style="Helper.TLabel",
        ).grid(row=1, column=0, columnspan=2, sticky="w", pady=(0, 6))

        self.app.block_images_check = ttk.Checkbutton(
            advanced_block,
            text="Block images for faster loads",
        ...
        ttk.Label(
            advanced_block,
            text="When enabled, image loading is disabled.",
            style="Helper.TLabel",
        ).grid(row=3, column=0, columnspan=2, sticky="w", pady=(0, 8))

        ttk.Label(
            advanced_block,
            text="User-Agent list (one per line)",
            background=self.colors["panel"],
            foreground=self.colors["text"],
        ...
        ttk.Label(advanced_block, text="Leave empty for defaults.", style="Helper.TLabel").grid(
        ...
        ttk.Label(
            advanced_block,
            text="Vote selectors (CSS or XPath per line)",
            background=self.colors["panel"],
            foreground=self.colors["text"],
        ...
        ttk.Label(
            advanced_block,
            text="Example: css:a[data-action='vote'] or xpath://button[contains(.,'vote')]",
            style="Helper.TLabel",
        ...
        log_frame = ttk.LabelFrame(self, text="Log", style="Panel.TFrame", padding=12)
        ...
        clear_btn = ttk.Button(
            log_frame, text="Clear Log", command=self.app.clear_log, style="Ghost.TButton"
        )
        clear_btn.grid(row=2, column=0, sticky="e")

        actions_frame = ttk.LabelFrame(self, text="Actions", style="Panel.TFrame", padding=12)
        ...
        self.app.start_btn = ttk.Button(
            controls, text="Start", command=self.app.start_bot, style="Accent.TButton"
        )
        ...
        self.app.preflight_btn = ttk.Button(
            controls, text="Preflight", command=self.app.run_preflight, style="Ghost.TButton"
        )
        self.app.preflight_btn.grid(row=0, column=2, padx=6, pady=6, sticky="ew", ipady=4)
        self.app.logs_btn = ttk.Button(
            controls, text="Open Logs", command=self.app.open_logs, style="Ghost.TButton"
        )
        self.app.logs_btn.grid(row=0, column=3, padx=6, pady=6, sticky="ew", ipady=4)
        self.app.reset_btn = ttk.Button(
            controls, text="Reset Counters", command=self.app.reset_counters, style="Ghost.TButton"
        )
